...
export const usePersistedTimer = ({
  ...
  const calculateTimeLeft = useCallback((startTime: number, duration: number): number => {
  ...
  const pause = useCallback(() => {
    ...
    if (startTimeRef.current) {
      const remaining = calculateTimeLeft(startTimeRef.current, durationRef.current);
      saveTimerState({
        startTime: Date.now(),
        duration: remaining,
        isRunning: false,
  ...
  useEffect(() => {
    ...
    return clearTimer;
  }, [isRunning, timeLeft, isCompleted, clearTimer, calculateTimeLeft, handleComplete]);

  // Load saved state on mount
  useEffect(() => {
    const savedState = loadTimerState();
    if (savedState && !isCompleted) {
      const remaining = calculateTimeLeft(savedState.startTime, savedState.duration);
  ...
  const progressPercentage = durationRef.current > 0 
