...
export const ShortcutsSection = React.forwardRef<HTMLElement, ShortcutsSectionProps>(
  ({ id }, ref) => {
  const [isEditing, setIsEditing] = useState(false)
  const [isLoading, setIsLoading] = useState(true)
  const [isSaving, setIsSaving] = useState(false)
  const [shortcuts, setShortcuts] = useState<KeyboardShortcut[]>(defaultShortcuts)
  const [originalShortcuts, setOriginalShortcuts] = useState<KeyboardShortcut[]>(defaultShortcuts)
      ...
      if (response.ok) {
        ...
        if (data.keyboardShortcuts) {
          const updatedShortcuts = defaultShortcuts.map(shortcut => {
             const saved = data.keyboardShortcuts.find((s: { id: string; keys: string }) => s.id === shortcut.id)
  ...
  const handleSave = async () => {
    setIsSaving(true)
    try {
      const shortcutsData = shortcuts.map(s => ({ id: s.id, keys: s.currentKeys }))
      const response = await fetch('/api/me/settings', {
        method: 'PATCH',
  ...
  const handleShortcutChange = (id: string, keys: string) => {
    setShortcuts(prev => prev.map(s => s.id === id ? { ...s, currentKeys: keys } : s))
  ...
  const handleResetAll = () => {
    setShortcuts(prev => prev.map(s => ({ ...s, currentKeys: s.defaultKeys })))
    if (!isEditing) setIsEditing(true)
  }
                  ...
                  <div key={shortcut.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <Label className="text-sm font-medium">{shortcut.action}</Label>
