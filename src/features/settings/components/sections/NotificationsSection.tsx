...
import { Pencil, X, Check, Plus, Trash2, Bell, Mail, Smartphone } from 'lucide-react'
import { toast } from 'sonner'
import type { NotificationPreferences, WorkoutReminder } from '@/features/settings/types'
...
export const NotificationsSection = React.forwardRef<HTMLElement, NotificationsSectionProps>(
  ({ id }, ref) => {
  const [isEditing, setIsEditing] = useState(false)
  const [editedData, setEditedData] = useState<NotificationPreferences | null>(null)
  const [newReminder, setNewReminder] = useState<Partial<WorkoutReminder> | null>(null)
  ...
  const { data: reminders, isLoading: remindersLoading } = useQuery<WorkoutReminder[]>({
    queryKey: ['reminders'],
    queryFn: async () => {
  ...
  const createReminderMutation = useMutation({
    mutationFn: async (data: Omit<WorkoutReminder, 'id' | 'userId' | 'createdAt' | 'updatedAt'>) => {
      const response = await fetch('/api/me/reminders', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
  ...
  const handleCreateReminder = () => {
    if (newReminder && newReminder.dayOfWeek !== undefined && newReminder.timeLocal) {
            ...
            <div className="space-y-4">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => {
                    if (isEditing && editedData) {
                      setEditedData({ ...editedData, workoutRemindersEmail: checked })
                    }
                  }}
                  disabled={!isEditing}
                />
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => {
                    if (isEditing && editedData) {
                      setEditedData({ ...editedData, productUpdatesEmail: checked })
                    }
                  }}
                  disabled={!isEditing}
                />
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => {
                    if (isEditing && editedData) {
                      setEditedData({ ...editedData, pushWorkoutReminders: checked })
                    }
                  }}
                  disabled={!isEditing}
                />
              ...
              {remindersLoading ? (
                ...
                <>
                  {(reminders || []).map((reminder) => (
                    <div key={reminder.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <h4 className="font-medium">
                            {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][reminder.dayOfWeek]} Reminder
                          </h4>
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => reminder.id && deleteReminderMutation.mutate(reminder.id)}
                        disabled={deleteReminderMutation.isPending || !reminder.id}
                      >
                  ...
                  {!newReminder ? (
                    <Button
                      variant="outline"
                      onClick={() => setNewReminder({ dayOfWeek: 1, timeLocal: '09:00', enabled: true })}
                      className="w-full"
                    >
                    ...
                    <div className="space-y-4 p-4 border rounded-lg bg-muted/50">
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => setNewReminder({ ...newReminder, timeLocal: e.target.value })}
                          />
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          disabled={newReminder.dayOfWeek === undefined || !newReminder.timeLocal || createReminderMutation.isPending}
                        >
                          <Check className="h-4 w-4 mr-2" />
                          Create
                        </Button>
                        <Button variant="outline" onClick={() => setNewReminder(null)}>
                          <X className="h-4 w-4 mr-2" />
                          Cancel
                        </Button>
                      </div>
